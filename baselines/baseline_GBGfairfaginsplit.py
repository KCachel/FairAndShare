...
def GBG_faginsplit(fairness, delta, L_items, L_scores, candidate_db, k):
    """
    ...
    for i in range(0,len(floors)):
        ...
        floors_i[i] = floors[i]
        k = floors[i]
        K_items_i, K_scores_i, _, _, _, _ = fagin_core(L_items, L_scores,floor_ids, candidate_db, floors_i, k)
...
def GBG_fagin_perfcountssplit(fairness, delta, L_items, L_scores, candidate_db, k):
    """
    GBG fairfagin baseline.
    :param fairness: String either 'proportional', 'equal' or 'rooney <x>', where <x> is an int.
    :param delta: Float fairness-utility parameter [0,1], where 0 is strict fairness and 1 is utility maximizing.
    ...
    for i in range(0,len(floors)):
        ...
        floors_i[i] = floors[i]
        k = floors[i]
        K_items_i, K_scores_i, sa_count_i, ra_count_i, total_seen_positions_i, seen_pos_i = fagin_core(L_items, L_scores,floor_ids, candidate_db, floors_i, k)
