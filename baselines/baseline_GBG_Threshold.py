import numpy as np
from baselines.baseline_greedyfmc import *
...
def GBG_threshold(fairness, delta, L_items, L_scores, candidate_db, k, t_style):
    """
    ...
    for i in range(0,len(floors)):
        ...
        k_ = floors[i]
        if k_ > 0:
            K_items_i, K_scores_i, _, _, _ , _= threshold_core(L_items, L_scores,floor_ids, candidate_db, floors_i, k_, t_style)
...
def GBG_threshold_perfcounts(fairness, delta, L_items, L_scores, candidate_db, k, t_style):
    """
    GBG fairfagin baseline.
    :param fairness: String either 'proportional', 'equal' or 'rooney <x>', where <x> is an int.
    :param delta: Float fairness-utility parameter [0,1], where 0 is strict fairness and 1 is utility maximizing.
    ...
    for i in range(0,len(floors)):
        ...
        floors_i[i] = floors[i]
        k_ = floors[i]
        K_items_i, K_scores_i, sa_count_i, ra_count_i, total_seen_positions_i, seen_pos_i = threshold_core(L_items, L_scores,floor_ids, candidate_db, floors_i, k_, t_style)
