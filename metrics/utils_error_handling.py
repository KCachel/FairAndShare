"""Functions for error handling of metrics
    References
    ----------
...
def check_pool_subset_groups(pool_items, pool_scores, pool_groups, subset_items, subset_scores, subset_groups):
    ...
    if (np.min(np.unique(subset_groups)) != 0) or (np.max(np.unique(subset_groups)) != len(
            np.unique(subset_groups)) - 1):  # check for consecutive group encoding in subset
        raise ValueError("Please represent subset groups via consecutive integers")
...
def check_pool_subset_items_groups(pool_items, pool_groups, subset_items, subset_groups):
    ...
    if (np.min(np.unique(subset_groups)) != 0) or (np.max(np.unique(subset_groups)) != len(np.unique(subset_groups))- 1): #check for consecutive group encoding in subset
        raise  ValueError("Please represent subset groups via consecutive integers")
