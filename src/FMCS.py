...
def fairness_criteria(candidate_db, k, fairness, delta):
    ...
    :return: floor_ids: A numpy array of group ids, floors: A numpy array indexed by group ids in floor_ids containing
                    the lower bound cardinality of each group.
    """

    num_items = np.shape(candidate_db)[1]
    groups = np.unique(candidate_db[1])  # unique groups
    cnt_per_group = np.asarray([np.count_nonzero(candidate_db[1] == i) for i in groups])  # item count per group

    if fairness == "proportional":
        group_kmin = np.round((cnt_per_group / num_items) * k)  # proportional representation
        if int(np.sum(group_kmin)) != k:  # check size of subset
            group_kmin[np.argmax(group_kmin)] += (k - np.sum(group_kmin))
            #raise ValueError("Fairest group k min values do not sum to k!")
        floors = np.asarray(np.floor((1 - delta) * group_kmin), dtype=int)

    if fairness == "equal":
        group_kmin = np.asarray([np.round((k / np.shape(groups)[0]))]*np.shape(groups)[0])  # equal representation
        if int(np.sum(group_kmin)) != k:  # check size of subset
            raise ValueError("Fairest group k min values do not sum to k!")
        floors = np.asarray(np.floor((1 - delta) * group_kmin), dtype=int)
    ...
    floor_ids = groups
    return floor_ids, floors


def rooney_calibrator(candidate_db, r):
    """
    Fairness calibrator for r-Rooney rule.
    :param candidate_db: A numpy array (shape 2 x n) of each item's group membership,
                    first row is item ids, second row is item group membership.
    :param r: Int the lower bound cardinality of each group.
    :return: floor_ids: A numpy array of group ids, floors: A numpy array indexed by group ids in floor_ids containing
                    the lower bound cardinality of each group.
    """
    num_items = np.shape(candidate_db)[1]
    groups = np.unique(candidate_db[1])  # unique groups
    cnt_per_group = np.asarray([np.count_nonzero(candidate_db[1] == i) for i in groups])  # item count per group
    if int(np.min(cnt_per_group)) < r:  # check size of subset
        raise ValueError("r value must be smaller than # items in a group")
    num_groups = len(groups)
    floors = np.tile(r, num_groups)
    floor_ids = groups
...
def ThresholdFMCS(fairness, delta, L_items, L_scores, candidate_db, k, t_style):
    ...
    if t_style == "TA":
        ...
        if np.sum(floors) == k: #Perfect fairness mode
            ...
            while not (np.count_nonzero(S_grp_scores >= threshold) == k): #k items each >= threshold
                ...
                for seen_item_id in range(0, len(unique_items)):
                        ...
                        if seen_item in S_grp_items[group_id]:
                            pass
                        # if nothing in S or less than floor items add it
                        elif np.count_nonzero(np.isfinite(S_grp_scores[group_id])) < grp_floor: #CHANGE TO LESS THAN FLOOR ITEMS
                            ...
                            if min_score_for_grp < seen_item_score:
                                indx_min = np.argwhere(S_grp_scores[group_id] == min_score_for_grp)[0][0] #index of min item
        ...
        if np.sum(floors) < k:  # Slack mode
            ...
            while not (np.count_nonzero(S_slack_scores >= threshold) == slack) or not(np.count_nonzero(S_grp_scores >= threshold) == sum_floors):
                ...
                for seen_item_id in range(0, len(unique_items)):
                    ...
                    elif np.count_nonzero(np.isfinite(S_grp_scores[group_id])) < grp_floor and seen_in_group_bool == False:
                        indx_to_fill = np.argwhere(np.isnan(S_grp_scores[group_id]))[0]
                        S_grp_items[group_id][indx_to_fill] = seen_item
                        S_grp_scores[group_id][indx_to_fill] = seen_item_score
...
def ThresholdFMCS_perfcounts(fairness, delta, L_items, L_scores, candidate_db, k, t_style):
    ...
    :param fairness: String either 'proportional', 'equal' or 'rooney <x>', where <x> is an int.
    :param delta: Float fairness-utility parameter [0,1], where 0 is stric fairness and 1 is utility maximizing.
    :param L_items: A numpy array (shape n x m) of the items in the sorted list.
    :param L_scores: A numpy array (shape n x m) of the items' scores in the sorted list.
    :param candidate_db: A numpy array (shape 2 x n) of each item's group membership,
                    first row is item ids, second row is item group membership
